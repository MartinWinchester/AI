...
def calc_fitness(agents):
    ...
    return sorted(fitness_tuples, key=lambda x: x[0])
...
class Utils:
    ...
    def natural_selection(self, fitness_tuples, num, mutation):
        ...
        for i in range(len(scores)):
            for j in range(int(scores[i])):
                self.mating_pool.append(fitness_tuples[1])
    ...
    def recombine(self, num, mutation):
        ...
        for i in range(num):
            ...
            mutate_gene_num = random.randint(round(mutate_gene_num-0.1*mutate_gene_num),
                                             round(mutate_gene_num+0.1*mutate_gene_num))
